...
export class Batsman extends Player {
  ...
  playInnings(runs, balls) {
    const validBattingStyle = ["right-hand", "left-hand"];
    if (!validBattingStyle.includes(this.battingStyle)) {
      return null;
    }
  ...
  getProfile() {
    const totalRuns = this.innings.reduce((sum, innings) => sum + innings.runs, 0);
...
export class Bowler extends Player {
  ...
  bowlSpell(wickets, runsConceded, overs) {
    const validBowlingStyle = ["fast", "spin", "medium"];
    if (!validBowlingStyle.includes(this.bowlingStyle)) {
      return null;
    }
  ...
  getProfile() {
    const totalWickets = this.spells.reduce((sum, spell) => sum + spell.wickets, 0);
